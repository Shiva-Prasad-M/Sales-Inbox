...
const CANDIDATE_ID = "medashivaprasad123@gmail.com";
...
export default function App() {
  ...
  function handlePaste() {
    try {
      ...
      if (!Array.isArray(arr))
        throw new Error("Expected an array of emails or {emails:[...]}");
      setEmails(arr);
      setError(null);
    } catch (e) {
  ...
  async function handleIngest() {
    ...
    const t0 = Date.now();
    try {
      const ingestBase = API_BASE.replace(/\/api$/, "") || "";
      const r = await fetch(`${ingestBase}/ingest`, {
        method: "POST",
  ...
  async function handleChat(e) {
    ...
    try {
      ...
      setChat((c) => [
        ...c,
        { role: "bot", text: data.answer, data: data.supporting_data },
      ]);
    } catch (err) {
  ...
  return (
    <div className="app">
      ...
      <div className="card">
        ...
        {ingestTime && (
          <div className="success">
            ✓ Processed {ingestTime.processed}, created{" "}
            {ingestTime.tasks_created}, updated {ingestTime.tasks_updated},
            skipped {ingestTime.skipped}
            {ingestTime.errors?.length
              ? `, errors: ${ingestTime.errors.join("; ")}`
              : ""}{" "}
            in {ingestTime.ms}ms
          </div>
        )}
      ...
      <div className="card">
        <h2>
          2. Raw Email Table{" "}
          <span className="muted">({emails.length} emails)</span>
        </h2>
        {emails.length > 0 ? (
        ...
        {stats ? (
          <div className="grid">
            <div className="stat">
              ...
              <div className="label">Emails processed</div>
            </div>
            <div className="stat">
              <div className="num">{stats.created}</div>
            ...
            <div className="stat">
              <div className="num">
                ₹{(stats.total_open_rfp_value || 0).toLocaleString("en-IN")}
              </div>
              <div className="label">Open RFP value</div>
            </div>
          </div>
        ) : (
      ...
      <div className="card">
        ...
        <div className="chat">
          <div className="chat-msgs">
            {chat.length === 0 && (
              <div className="muted">
                Ask questions about the ingested data, e.g. "How many enterprise
                RFPs?", "Show everything in triage and why".
              </div>
            )}
            {chat.map((m, i) => (
          ...
          <form className="chat-input" onSubmit={handleChat}>
            ...
            />
            <button className="btn" disabled={chatLoading}>
              Ask
            </button>
          </form>
        </div>
